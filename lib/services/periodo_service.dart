...
class PeriodoService {
  static const String resource = "periodos/";
  ...
  Future<List<Periodos>> getAll() async {
    ...
    List<Periodos> result = [];

    List<dynamic> jsonList = json.decode(response.body);

    for (var jsonMap in jsonList) {
      result.add(Periodos.fromMap(jsonMap));
    }

    return result;
  }

  Future<List<Periodos>> getId(String id) async {
    String user = await getUser();
    String password = await getPassword();
    final bytes = utf8.encode('$user:$password');
    final base64Str = base64.encode(bytes);

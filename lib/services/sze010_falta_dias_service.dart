...
import '../models/sze010_falta_dias.dart';

class Srh010FaltaDiasService {
  static const String resource = "falta_dias/";
  ...
  Future<List<Sze010FaltaDias>> getAll() async {
    ...
    List<Sze010FaltaDias> result = [];

    List<dynamic> jsonList = json.decode(response.body);

    for (var jsonMap in jsonList) {
      result.add(Sze010FaltaDias.fromMap(jsonMap));
    }

    return result;
  }

  Future<List<Sze010FaltaDias>> searchMat(String mat) async {
    String user = await getUser();
    String password = await getPassword();
    final bytes = utf8.encode('$user:$password');
    final base64Str = base64.encode(bytes);
    http.Response response = await client.get(
      Uri.parse("${WebClient.url}$resource?search=$mat"),
      headers: {
        'Content-type': 'application/json',
        'Authorization': 'Basic $base64Str',
      },

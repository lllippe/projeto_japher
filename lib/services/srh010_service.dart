...
class Srh010Service {
  static const String resource = "ferias_colaborador_periodo/";
  ...
  Future<List<Srh010>> getAll(String matricula, String periodo) async {
    String user = await getUser();
    String password = await getPassword();
    final bytes = utf8.encode('$user:$password');
    final base64Str = base64.encode(bytes);
    http.Response response = await client.get(
      Uri.parse("${WebClient.url}$resource$matricula&$periodo"),
      headers: {
    ...
    for (var jsonMap in jsonList) {
      result.add(Srh010.fromMap(jsonMap));
    }

    return result;
  }

  Future<List<FeriasSze>> getId(String matricula) async {
    String user = await getUser();
    String password = await getPassword();
    final bytes = utf8.encode('$user:$password');
    final base64Str = base64.encode(bytes);
    http.Response response = await client.get(
      Uri.parse("${WebClient.url}ferias_todas/?search=$matricula"),
      headers: {
        'Content-type': 'application/json',
        'Authorization': 'Basic $base64Str',
      },

...
class Szh010Service {
  static const String resource = "requisicao_ferias/";
  ...
  Future<List<Szh010>> getGroup(int depto, int group) async {
    String user = await getUser();
    String password = await getPassword();
    final bytes = utf8.encode('$user:$password');
    final base64Str = base64.encode(bytes);
    http.Response response = await client.get(
      Uri.parse("${WebClient.url}requisicao_group/$depto&$group"),
      headers: {
        'Content-type': 'application/json',
        'Authorization': 'Basic $base64Str',
      },
  ...
  Future<List<Szh010>> searchUnique(String mat, String initialDate, String finalDate) async {
    String user = await getUser();
    String password = await getPassword();
    final bytes = utf8.encode('$user:$password');
    final base64Str = base64.encode(bytes);
    http.Response response = await client.get(
      Uri.parse("${WebClient.url}requisicao_ferias/?search=$mat,$initialDate,$finalDate"),
      headers: {
        'Content-type': 'application/json',
        'Authorization': 'Basic $base64Str',
      },
